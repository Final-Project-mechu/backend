...
document.addEventListener('DOMContentLoaded', function() {
  // 카테고리 버튼에 대한 이벤트 리스너
  document.querySelectorAll(".filter-button, .sub-filter-button").forEach(button => {
      button.addEventListener("click", function() {
          // 버튼의 data-category-id 값을 가져와 selectedCategoryId에 저장합니다.
          selectedCategoryId = parseInt(button.getAttribute('data-category-id'), 10);
          console.log(JSON.stringify({category_id: selectedCategoryId})); // 선택된 카테고리 ID를 JSON 형태로 콘솔에 출력합니다.
      });
  });

  // sendRequest 버튼에 대한 이벤트 리스너
  document.querySelector(".button-container2 .button").addEventListener("click", function() {
